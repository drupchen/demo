...
export default function PlayerTab({
  ...
  const transcript = useMemo(() => {
    return activeCommentarySegments.map((segment) => {
      ...
      const endTimeMs = segment.end
        ? parseToMs(segment.end)
        : startTimeMs + 10000;
  ...
  const activeSegIndex = useMemo(() => {
    const idx = transcript.findIndex(
      (seg) =>
        audio.currentTimeMs >= seg.startTimeMs &&
        audio.currentTimeMs < seg.endTimeMs,
    );
    if (idx >= 0) return idx;
    const past = transcript.filter(
      (seg) => seg.startTimeMs <= audio.currentTimeMs,
    );
  ...
  const totalDurationMs =
    audio.durationMs ||
    (transcript.length > 0 ? transcript[transcript.length - 1].endTimeMs : 0);
  ...
  const { groupSessions, currentGroupIndex } = useMemo(() => {
    ...
    const sessions = allCommentaryIds.filter(
      (id) => getCommentaryGroup(id) === currentGroup,
    );
  ...
  const hasNextSession =
    currentGroupIndex >= 0 && currentGroupIndex < groupSessions.length - 1;
  ...
  const buildSegmentUrl = useCallback(
    (seg) => {
  ...
  useLayoutEffect(() => {
    ...
    if (top + pop.height > window.innerHeight - pad)
      top = window.innerHeight - pad - pop.height;
    if (top < pad) top = pad;
  ...
  // A timeline jump teleports (no animation) and overrides the scroll lock.
  useEffect(() => {
    if (activeSegIndex < 0 || !transcriptRef.current) return;
    const isJump = jumpRef.current;
    if (!isJump && Date.now() - userScrolledAt < 8000) return;
  ...
  // Pass the timeline-jump flag so the root text teleports instead of animating.
  useEffect(() => {
    if (activeSegIndex >= 0 && transcript[activeSegIndex]) {
      onSegmentClick?.(transcript[activeSegIndex], jumpRef.current);
    }
        ...
        <div className="flex-1 flex flex-col px-5 py-4">
          ...
          <button
            onClick={() =>
              ...
              onCommentarySelect(noSessionMessage.prevSession)
            }
          ...
          <button
            onClick={() =>
              ...
              onCommentarySelect(noSessionMessage.nextSession)
            }
  ...
  return (
    <div className="flex flex-col h-full -mx-5 -mb-5">
            ...
            <div className="h-1.5 rounded-full relative r-progress-track">
              <div
                ...
                style={{
                  width:
                    totalDurationMs > 0
                      ? `${(audio.currentTimeMs / totalDurationMs) * 100}%`
                      : "0%",
                }}
              />
            </div>
        ...
        {transcript.length > 0 && (
          <div className="flex h-2 rounded-full overflow-hidden gap-[1px] mt-2">
            {transcript.map((seg, idx) => {
              const widthPct =
                totalDurationMs > 0
                  ? (seg.durationMs / totalDurationMs) * 100
                  : 0;
      ...
      <div
        ref={transcriptRef}
        className={`flex-1 overflow-y-auto px-5 py-4 space-y-1 ${slideClass}`}
      >
        {/* Previous session button — always visible, grayed when unavailable */}
        <button
          onClick={() =>
            hasPrevSession &&
            onCommentarySelect(groupSessions[currentGroupIndex - 1])
          }
        ...
        {transcript.map((seg, idx) => {
          ...
          return (
            ...
            >
              <span
                className={`${inter.className} text-[9px] font-medium r-text-secondary opacity-50 mr-1.5`}
              >
                {idx + 1}
              </span>
              {isActive && (
                <span
                  className={`${inter.className} text-[10px] font-bold uppercase tracking-wider r-text-accent mr-2`}
                >
                  Current
                </span>
              )}
              <span className={`${uchen.className} leading-relaxed r-text`}>
                {seg.syllables.map((syl, i) => {
                  const sylStyle =
                    sidebarSizes?.[syl.size?.toUpperCase()] ||
              ...
              <span
                className={`${inter.className} inline-flex ml-2 text-[10px] font-medium px-1.5 py-0.5 rounded-full align-middle r-badge`}
              >
        ...
        createPortal(
          ...
          >
            <div className="bg-white/50 backdrop-blur-md p-4">
              <p
                className={`${inter.className} text-[10px] uppercase tracking-widest font-bold r-text-accent mb-3`}
              >
                Link copied
              </p>
              <div className={`${inter.className} text-[11px] space-y-1 mb-4`}>
                {teachingTitle && (
                  <p>
                    <span className="r-text-secondary">Teaching: </span>
                    <span className="r-text-primary font-medium">
                      {teachingTitle}
                    </span>
                  </p>
                )}
                <p>
                  <span className="r-text-secondary">Instance: </span>
                  <span className="r-text-primary font-medium">
                    {instanceId}
                  </span>
                </p>
                <p>
                  <span className="r-text-secondary">Session: </span>
                  <span className="r-text-primary font-medium">
                    {shortSessionId}
                  </span>
                </p>
                <p>
                  <span className="r-text-secondary">Segment: </span>
                  <span className="r-text-primary font-medium">
                    {contextMenu.segIdx + 1}
                  </span>
                </p>
              </div>
              <button
    ...
    <button
      onClick={() => {
        ...
        const el = document.getElementById(
          `seg-${transcript[activeSegIndex]?.id}`,
        );
